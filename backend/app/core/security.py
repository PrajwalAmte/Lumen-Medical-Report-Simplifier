...
# Magic bytes used to verify file content matches its declared extension
MAGIC_BYTES = {
    'pdf': b'%PDF',
    'jpeg': [b'\xFF\xD8\xFF'],
    'png': b'\x89PNG',
...
def validate_file(file: UploadFile):
    """Raise 400 if file extension is not in the allowed list."""
...
def validate_file_magic_bytes(file_content: bytes, filename: str):
    """Raise 400 if file bytes don't match the declared file extension."""
    ...
    if ext == 'pdf':
        ...
        is_valid_jpeg = any(
            file_content.startswith(p) for p in MAGIC_BYTES['jpeg']
        )
